...
impl SequencerDB for SequencerRocksDB {
    ...
    where
        Self: Sized, {
        let db = config
            .ok_or(TwineSequencerError::Other(format!("config cannot be None")))?
            .db_path
            .ok_or(TwineSequencerError::Other(format!("db path is not set")))?;
    ...
    async fn insert(
        ...
        let ns = self
            .db
            .cf_handle(&ns)
            .ok_or(TwineSequencerError::SequencerDBError(format!(
