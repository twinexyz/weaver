...
impl Hasher {
    pub fn hash(&mut self, val: &[u8]) { self.hasher.update(val); }
    ...
    pub fn result(self) -> Hash { Hash(self.hasher.finalize().into()) }
}
...
impl Hash {
    ...
    pub fn to_bytes(self) -> [u8; HASH_BYTES] { self.0 }
}
...
/// Return a Sha256 hash for the given data.
pub fn hash(val: &[u8]) -> Hash { hashv(&[val]) }
