...
pub enum InstructionError {
    ...
    /// An initialize instruction was sent to an account that has already been
    /// initialized.
    ...
    /// Program's instruction lamport balance does not equal the balance after
    /// the instruction
    ...
    /// Program other than the account's owner changed the size of the account
    /// data
    ...
    /// The same account was multiply passed to an on-chain program's
    /// entrypoint, but the program modified them differently.  A program
    /// can only modify one instance of the account because the runtime
    /// cannot determine which changes to pick or how to merge them if both are
    /// modified
    #[error("instruction modifications of multiply-passed account differ")]
    DuplicateAccountOutOfSync,

    /// Allows on-chain programs to implement program-specific error types and
    /// see them returned by the Solana runtime. A program-specific error
    /// may be any type that is represented as or serialized to a u32
    /// integer.
    #[error("custom program error: {0:#x}")]
    Custom(u32),

    /// The return value from the program was invalid.  Valid errors are either
    /// a defined builtin error value or a user-defined error in the lower
    /// 32 bits.
