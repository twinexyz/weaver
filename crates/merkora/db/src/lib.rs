...
pub async fn run(db: PgPool, receiver: &mut mpsc::Receiver<L1MessageDetails>) {
...
pub async fn create_messages_table(pool: &PgPool) -> Result<()> {
    let create_table = r#"
        CREATE TABLE IF NOT EXISTS messages (
            nonce BIGINT NOT NULL,                                                                   -- Nonce value (unique within a chain)
            message_type VARCHAR(20) NOT NULL,                                                       -- Type of message ('deposit', 'withdraw', 'general')
            chain_id BIGINT NOT NULL,                                                                -- Chain ID
            slot_or_block_number BIGINT NOT NULL,                                                    -- Slot (Solana)or block number (Ethereum)
            bank_hash_or_receipt_root BYTEA NOT NULL,                                                -- Bank hash (Solana) or receipt root (Ethereum)
            public_values BYTEA NOT NULL,                                                            -- Public values (binary data)
            proof BYTEA NOT NULL,                                                                    -- Proof data (binary data)
            processed BOOLEAN NOT NULL DEFAULT FALSE,                                                -- Whether the message has been processed
            created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,                                 -- Timestamp of insertion
            PRIMARY KEY (chain_id, nonce, message_type),                                             -- Composite primary key
            CONSTRAINT valid_message_type CHECK (message_type IN ('deposit', 'withdraw', 'general')) -- Validate message types
        );
    "#;

    // index for unprocessed messages
    let create_idx1 =
        r#"CREATE INDEX IF NOT EXISTS idx_chain_processed ON messages (chain_id, processed);"#;

    // index for filtering by message type
    let create_idx2 = r#"CREATE INDEX IF NOT EXISTS idx_chain_message_type ON messages (chain_id, message_type);"#;

    // index for nonce-based lookups
    let create_idx3 =
        r#"CREATE INDEX IF NOT EXISTS idx_chain_nonce ON messages (chain_id, nonce);"#;
...
/// Fetches the oldest unprocessed `slot_or_block_number` from the `messages`
/// table.
pub async fn fetch_oldest_unprocessed_slot_or_block_number(
    pool: &PgPool,
    chain_id: u64,
) -> Result<u64> {
    let query = r#"
        ...
        ORDER BY slot_or_block_number ASC
        LIMIT 1;
    "#;
