...
pub async fn handle_ethereum_precompile(
    ...
    let proof_height = 9226737;
    let state_root =
        B256::from_str("0x470062716b296da00483a73c4a9590845954f6c5a06c678c65013cd4be6c8b97")
            .unwrap();

    // Read the proof file to a string
    let proof_string = fs::read_to_string("bin/devtest/res/storage_proof.json")
        .expect("Failed to read storage_proof.json");

    // Parse the string into a JSON value
    let proof_json: serde_json::Value =
        serde_json::from_str(&proof_string).expect("Failed to parse JSON from proof file");

    // Convert the JSON value to a compact string
    let compact_proof_string =
        serde_json::to_string(&proof_json).expect("Failed to create compact JSON string");

    // Convert the compact string to Bytes
    let serialized_proof = Bytes::from(compact_proof_string.into_bytes());
    ...
    let message_data = MessageData {
        ...
        chainId: 11155111,
        blockNumber: 9226728,
        fromAddress: String::from("0x5BA85D71ef9aE0EdC180a6bf5e65F0a749f062DC"),
    ...
    let txn = precompile_caller
        .handleEthereumProofAndTransactions(
            U256::from(proof_height),
            state_root,
            message_data.clone(),
            serialized_proof,
        )
    ...
    for log in receipt.logs() {
        ...
        info!(
            "Decoded log - chainId: {}, status: {}, nonce: {}, transactionOutput length: {}",
            decoded_log.inner.chainId,
            decoded_log.inner.status,
            decoded_log.inner.nonce,
            decoded_log.inner.transactionOutput.len()
        );

        // Check if transactionOutput is empty
        if decoded_log.inner.transactionOutput.is_empty() {
            // Try to decode as error message if the output is empty
            return Err(eyre::eyre!("Transaction output is empty - precompile returned no data. This typically means the precompile failed validation (e.g., invalid proof or state root mismatch)"));
        }

        // If there are bytes but they might be an error message, check first
        if decoded_log.inner.transactionOutput.len() < 100 {
            // Small output might be an error string
            let potential_error = String::from_utf8_lossy(&decoded_log.inner.transactionOutput);
            info!(
                "Potential error message from precompile: {}",
                potential_error
            );
        }
